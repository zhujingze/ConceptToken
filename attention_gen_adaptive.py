...
def llama_new_forward(
    ...
    **kwargs,
    ...
    c_sum = attn_weights_sink[:, :, -1, c_idx].sum(dim=-1).mean(dim=1)
    s_sum = attn_weights_sink[:, :, -1, s_idx].sum(dim=-1).mean(dim=1)
    n_sum = attn_weights_sink[:, :, -1, n_idx].sum(dim=-1).mean(dim=1)
    # ### 每个head
    ...
    if use_attn and not use_cfg:
        ...
        if token_weaken and wk_c:
            if ema:
                ...
                # 4. 应用掩码到所有heads的对应位置
                attn_slice = attn_weights[:, :, -1, token_weaken]  # 获取目标切片
                final_mask = final_mask.to(attn_slice.device)
                attn_slice = torch.where(final_mask, -1e2 * torch.ones_like(attn_slice), attn_slice)
                attn_weights[:, :, -1, token_weaken] = attn_slice
                ...
                final_mask = final_mask.expand(-1, attn_weights.size(1), len(token_weaken))  # [batch, heads, , n_weaken]
                ...
                attn_slice = torch.where(final_mask, -1e2 * torch.ones_like(attn_slice), attn_slice)
                attn_weights[:, :, -1, token_weaken] = attn_slice
                #print(attn_weights[:, :, -1, token_weaken])
                #attn_weights[:, :, first_token_idx:, token_weaken] = -1e2
        ...
        if sink:
            ...
            if over_threshold.any():
                ...
                #print(nonzero_indices.size())
                col_indices = nonzero_indices[:, -1]         # 提取第三列（cols维度）
...
def llama_modify_adaptive(
    ...
    for i in range(start_layer, end_layer + 1):
        ...
        self_attn.model_type = model_type
        self_attn.use_cfg = use_cfg
        self_attn.wk_c = wk_c
        self_attn.stored_tokens = []
