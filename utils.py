...
###
def determine_start_symbol(model_type):
...
def is_new_word_start(token_text, model_type, start_sym):
    punctuation = set(string.punctuation)
    ###
    #print(token_text[0],'第一个字符')
    # 检查特殊符号
    special_tokens = ['<0x0A>', '<s>', '<|begin_of_text|>']
    if token_text in special_tokens:
        return True
    # 确定起始符号
    has_start_sym = start_sym and token_text.startswith(start_sym)
    stripped_part = token_text.lstrip(start_sym) if start_sym else token_text
    # 带起始符号的标点或普通词
    if has_start_sym:
        return True
    # 不带起始符号的标点
    #if all(c in punctuation for c in token_text):
    if token_text[0] in punctuation:
        ###
        return True
    return False
    ...
    if is_new_word:
        if stored_tokens:
            ...
            for i, tok in enumerate(stored_tokens):
                if i == 0 and start_sym and tok[0].startswith(start_sym):
                    merged_word += tok[0].lstrip(start_sym)
                else:
                    merged_word += tok[0]
            # 词性标注
            #tagged = pos_tag([merged_word], tagset='universal')
            tagged = pos_tag([merged_word])
            if tagged:
                word = tagged[0][0]
                tag = tagged[0][1]
                #print('ori',word,tag)
                ...
                if not ((word in ['<|begin_of_text|>','<s>', None, '"', ':', '.', ',', '?', '<0x0a>', ''] or word.startswith('_') or word.startswith('Ġ')) or (tag in ['IN', 'DT', 'TO', 'MD', 'CC'] or tag.startswith('W') or word in all_words)):
                    if (tag.startswith('N') and word != 'which') or tag in ['CD', 'PRP', 'PRP$', 'POS'] or tag.startswith('J') or tag.startswith('V'):
                    #if pos in ['NOUN', 'ADJ', 'VERB', 'PROPN', 'NUM', 'SYM']:
                        # 计算索引
                        #print('in',word,tag)
        ...
        # 排除特殊符号和纯标点
        if not (next_token_text in ['<0x0A>', '<s>'] or all(c in punctuation for c in next_token_text)):
            model.layers[attn_layers[0]].self_attn.stored_tokens.append(next_token_text)
    else:
        # 继续当前词
        model.layers[attn_layers[0]].self_attn.stored_tokens.append(next_token_text)
...
class GenerationMixin:
    ...
    def generate(
        ...
        attn_decoding: Optional[bool] = None,
        ave: Optional[bool] = None,
        ...
        including_answers: Optional[bool] = None,
        ...
        negative_prompt_ids: Optional[torch.Tensor] = None,
        negative_prompt_attention_mask: Optional[torch.Tensor] = None,
        sled_decoding: Optional[bool] = None,
        ...
        **kwargs,
        ...
        if generation_mode == GenerationMode.GREEDY_SEARCH and dola_decoding:
            if generation_config.num_return_sequences > 1:
                raise ValueError(
        ...
        elif generation_mode == GenerationMode.GREEDY_SEARCH and attn_decoding:
            if generation_config.num_return_sequences > 1:
                raise ValueError(
                    f"num_return_sequences has to be 1, but is {generation_config.num_return_sequences} when doing"
                    " greedy search."
            ...
            # 11. run greedy search
            return self.attn_greedy_decode(
                input_ids,
                logits_processor=prepared_logits_processor,
                stopping_criteria=prepared_stopping_criteria,
                pad_token_id=generation_config.pad_token_id,
        ...
        elif generation_mode == GenerationMode.GREEDY_SEARCH and sled_decoding:
            ...
            # 11. run greedy search
            return self.sled_greedy_decode(
                input_ids,
                logits_processor=prepared_logits_processor,
                stopping_criteria=prepared_stopping_criteria,
                pad_token_id=generation_config.pad_token_id,
    ...
    def attn_greedy_decode(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # next_token_text = self.model.layers[attn_layers[0]].self_attn.tokenizer.convert_ids_to_tokens(next_tokens[0], skip_special_tokens=False)
            
            # self.model.layers[attn_layers[0]].self_attn.stored_tokens.append(next_tokens)
            #print('new',next_tokens)
            if including_answers:
                
                process_new_token(next_tokens, self.model, self.model.layers[attn_layers[0]].self_attn.model_type, input_ids,attn_layers)
                
                token_weaken_new = self.model.layers[attn_layers[0]].self_attn.token_weaken
                for idx, i in enumerate(attn_layers):
                    if idx != 0:
                        if hasattr(self.model.layers[i].self_attn, "use_attn"):
                            #print("False?",self.model.layers[i].self_attn.use_cfg)
            ...
            # 获取需要弱化的 token 索引
            # weaken_indices = self.model.layers[attn_layers[0]].self_attn.token_weaken

            # # 确保索引张量在 CUDA 上
            # weaken_indices = torch.tensor(
            #     self.model.layers[attn_layers[0]].self_attn.token_weaken,
            #     dtype=torch.long,  # 确保是整数类型
            #     device=input_ids.device  # 直接指定设备
            # )

            # # 提取对应位置的 token ID（假设 input_ids 形状为 [1, seq_len]）
            # if input_ids.dim() == 2 and input_ids.size(0) == 1:
            #     weaken_token_ids = input_ids[0, weaken_indices]
            # else:
            #     raise ValueError("input_ids 应为单样本二维张量")

            # # 转换为 Python 列表（可选）
            # weaken_token_ids_list = weaken_token_ids.cpu().tolist()

            # print('added',self.model.layers[attn_layers[0]].self_attn.tokenizer.convert_ids_to_tokens(weaken_token_ids_list, skip_special_tokens=False))
            ...
            if streamer is not None:
                streamer.put(next_tokens.cpu())
            model_kwargs = self._update_model_kwargs_for_generation(
                outputs_ori, model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
            )
    ...
    def dola_greedy_decode(
        ...
        while True:
            ...
            if base_layer is not None:
                ...
                softmax_mature_layer = F.softmax(dict_outputs[mature_layer][:, -1, :],
                ...
                log_softmax_mature_layer = F.log_softmax(dict_outputs[mature_layer][:, -1, :],
                                                         dim=-1)  # shape: (batch_size, num_features)
    ...
    def sled_greedy_decode(
        ...
        while True:
            ...
            if base_layer is not None:
                ...
                new_output_logits[:, -1, :] = hidden_states_seq_i_new.unsqueeze(dim=0)
